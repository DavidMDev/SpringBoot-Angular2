...
public class ChatSocketHandler implements WebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		if(username != "") {
			...
			for (WebSocketSession openSession : openSessions) {
				openSession.sendMessage(new TextMessage("/<" + now.getHours() + ":" + now.getMinutes()+" "  + username + "> : connected to the chat"));
			}
	...
	public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
		...
		for (WebSocketSession openSession : openSessions) {
				...
				if(text.startsWith("/me ")) {
					openSession.sendMessage(new TextMessage("/*"  + sessionIdToUsername.get(session.getId()) + " " + text.replaceAll("/me", "")));
				} else {
					openSession.sendMessage(new TextMessage("/<" + now.getHours() + ":" + now.getMinutes()+" "  + sessionIdToUsername.get(session.getId()) + "> " + text));
				}
		}
	}
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus closeStatus) throws Exception {
		...
		Date now = new Date();
		for (WebSocketSession openSession : openSessions) {
			openSession.sendMessage(new TextMessage("/<" + now.getHours() + ":" + now.getMinutes()+" "  + sessionIdToUsername.get(session.getId()) + "> " + " : disconnected from the chat"));
		}
		sessionIdToUsername.remove(session.getId());
		session.close();
	}
